...
io.on("connection", (socket) => {
    socket.on("cliente-mensaje", async (message) => {
        ...
        console.log("Longitud de chat sin normalizar:", porc1);

        try {
            knex("messages")
                .insert(CHAT_DB)
                .then(() =>
                    console.log("Mensajes de chat guardados en sqlite3")
                )
                .then(() =>
                    fs.writeFile(`messages.txt`, JSON.stringify(CHAT_DB))
                )
                .then(() =>
            ...
            console.log("Longitud de chat normalizada:", porc2);
